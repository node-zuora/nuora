 ...
 * @return ZuoraObject
 */
Zuora.init = function (handlerInstance) {
    handler = handlerInstance;
    config = handler.config.zuora;
    log = handler.log;
    return new Zuora();
};
...
Zuora.prototype.parseWSDL = function (buffer) {//{{{
    ...
    if (null === matchObjects || null === matchModels || null === matchDefinitions) {
    ...
    bindBuilder = function (ref, type) {//{{{
        ...
        if (undefined !== zuora.config.wsdlMixins && undefined !== zuora.config.wsdlMixins[type]) {
