 ...
 * @return ZuoraObject
 */
Zuora.build = function (nuoraInstance) {
    nuora = nuoraInstance;
...
Zuora.prototype.initialize = function (wsdlPath) {//{{{
    ...
    fs.readFile(wsdlPath, function (err, data) {
        if (err) {
            log.info(err);
            throw new Error('Could not initialize WSDL: ' + wsdlPath);
        } else {
 ...
 * @param {function} callback - the callback function to be issued on success/error
 */
Zuora.prototype.query = (function () {
	
	var fieldFilter = function (str) {
	...
	return function (queryString, callback) {//{{{
		var zuora = this,
			xml = Soap.create(),
			fields = queryString.split(/(select | from)/g)[2].split(', ').map(fieldFilter),
...
var QueryHandler = function (zuora, fields, limit, callback) {
        ...
        that.responseHandler = function (err, data) {
            ...
            if (null !== data.result && undefined !== data.result.queryLocator) {
